...
import { AccOrderConversationMessageApi, AccOrderConversationMessage } from '@/api/gamer/accorderconversationmessage'

/** 陪玩订单会话消息 表单 */
defineOptions({ name: 'AccOrderConversationMessageForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await AccOrderConversationMessageApi.getAccOrderConversationMessage(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as AccOrderConversationMessage
    if (formType.value === 'create') {
      await AccOrderConversationMessageApi.createAccOrderConversationMessage(data)
      message.success(t('common.createSuccess'))
    } else {
      await AccOrderConversationMessageApi.updateAccOrderConversationMessage(data)
