...
import { OrderConversationApi, OrderConversation } from '@/api/gamer/orderconversation'

/** 服务订单会话 表单 */
defineOptions({ name: 'OrderConversationForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await OrderConversationApi.getOrderConversation(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as OrderConversation
    if (formType.value === 'create') {
      await OrderConversationApi.createOrderConversation(data)
      message.success(t('common.createSuccess'))
    } else {
      await OrderConversationApi.updateOrderConversation(data)
