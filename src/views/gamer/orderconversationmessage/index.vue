...
import OrderConversationMessageForm from './OrderConversationMessageForm.vue'

/** 服务订单会话消息 列表 */
defineOptions({ name: 'OrderConversationMessage' })
...
async function handleDelete(id: number) {
  try {
    ...
    // 发起删除
    await OrderConversationMessageApi.deleteOrderConversationMessage(id)
...
async function handleDeleteBatch() {
  try {
    // 删除的二次确认
    await message.delConfirm()
    await OrderConversationMessageApi.deleteOrderConversationMessageList(checkedIds.value)
    checkedIds.value = []
    message.success(t('common.delSuccess'))
    await getList()
  }
...
function handleRowCheckboxChange(records: OrderConversationMessage[]) {
...
async function handleExport() {
  try {
    ...
    const data = await OrderConversationMessageApi.exportOrderConversationMessage(queryParams)
    download.excel(data, '服务订单会话消息.xls')
  }
  catch {
  }
  finally {
...
<template>
    ...
    >
      ...
      <el-table-column label="内容" align="center" min-width="200">
        <template #default="{ row }">
          ...
          <el-image v-else-if="row.messageType === 2" :src="row.fileUrl" :preview-src-list="[row.fileUrl]" fit="cover" style="height: 60px; width: 60px" />
          <el-link v-else-if="row.messageType === 3" :href="row.fileUrl" :download="row.fileName || ''" target="_blank" type="primary">
            {{ row.fileName || '下载文件' }}
          </el-link>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <OrderConversationMessageForm ref="formRef" @success="getList" />
</template>
