...
import FighterDataDialog from './FighterDataDialog.vue'
import RoleMemberProfileDialog from './RoleMemberProfileDialog.vue'

/** 打手列表（只展示已通过的申请） */
defineOptions({ name: 'Fighter' })
...
async function getList() {
  loading.value = true
  try {
    // 强制固定 levelType=2、auditStatus=1，只返回已通过的打手
    const data = await LevelApplyApi.getLevelApplyPage({ ...queryParams, levelType: 2, auditStatus: 1 })
...
// 查看打手数据弹窗
const fighterDataDialogRef = ref()
function handleViewFighterData(row: LevelApply) {
  fighterDataDialogRef.value.open(row.userId, row.userNickname, 2)
}

// 当前票数调整（沿用 level-apply 的增加/减少票数接口）
...
async function handleToggleRoleBan(row: LevelApply) {
  ...
  try {
    await message.confirm(`确认${action}“${row.userNickname || row.userId}”的打手身份？`)
...
<template>
      ...
      <el-table-column label="操作" align="center" min-width="250px" fixed="right">
        <template #default="scope">
          ...
          <el-button
            ...
            link
            type="primary"
            @click="handleViewFighterData(scope.row)"
          >
            查看数据
          </el-button>
        </template>
  ...
  <!-- 查看打手数据弹窗 -->
  <FighterDataDialog ref="fighterDataDialogRef" />
