...
/** 用户投票选项 表单 */
defineOptions({ name: 'UserMomentVoteOptionForm' })
...
const formRules = reactive({
  ...
  optionText: [{ required: true, message: '选项内容不能为空', trigger: 'blur' }]
})
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await UserMomentVoteOptionApi.getUserMomentVoteOption(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as UserMomentVoteOption
    if (formType.value === 'create') {
      await UserMomentVoteOptionApi.createUserMomentVoteOption(data)
      message.success(t('common.createSuccess'))
    } else {
      await UserMomentVoteOptionApi.updateUserMomentVoteOption(data)
