...
import type { KeFuConversationRespVO } from '@/api/mall/promotion/kefu/conversation'
import type { KeFuMessageRespVO } from '@/api/mall/promotion/kefu/message'
...
async function getNewMessageList(val: KeFuConversationRespVO) {
  // 1. 缓存当前会话消息列表
...
const showTime = computed(() => (item: KeFuMessageRespVO, index: number) => {
  if (unref(messageList.value)[index + 1]) {
    const dateString = dayjs(unref(messageList.value)[index + 1].createTime).fromNow()
                ...
                <MessageItem :message="item">
                  <template v-if="KeFuMessageContentTypeEnum.TEXT === item.contentType">
                    <div
                      ...
                      class="h-1/1 w-full text-justify line-height-normal"
                    />
                  </template>
                </MessageItem>
                <!-- 图片消息 -->
