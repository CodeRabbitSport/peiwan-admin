...
/** 打开弹窗 */
async function open(id: number, selectedList?: any[]) {
  activityId.value = id
  resetForm()
...
/** 获取指定部门及其所有子部门的ID列表 */
function getChildDeptIds(deptId: number, deptList: any[]): number[] {
  const ids = [deptId]
...
/** 获取部门过滤后的用户列表 */
async function filterUserList(deptId?: number) {
...
/** 处理部门被点击 */
function handleNodeClick(row: { [key: string]: any }) {
...
<template>
  <Dialog v-model="dialogVisible" title="人员选择" width="800">
    <el-row v-loading="formLoading" class="gap-2">
      <el-col :span="6">
